...
<!-- image tron-->
@if(isset($category->image_tron) && !empty($category->image_tron))
<img src="{{ url('uploads/categories/tron/' . $category->image) }}" width="200" height="180"/>
@endif

<div class="form-group {{ $errors->has('image_tron') ? 'has-error' : ''}}">
    <label for="image_tron" class="control-label">{{ 'Hình ảnh tròn' }}</label>
    <input class="form-control" name="image_tron" type="file" id="image_tron" >
    {!! $errors->first('image_tron', '<p class="help-block">:message</p>') !!}
</div>
