        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                <div class="form-group {{ $errors->has('ship') ? 'has-error' : ''}}">
                    <label for="ship" class="control-label">{{ 'Giao hàng miễn phí?' }}</label>
                    <select id="ship" name="ship" class="form-control" >
                        <option value="0" {{ $formMode == 'create'?"selected":($product-> ship == 0?"selected":"") }} >Không</option>
                        <option value="1" {{ $formMode == 'edit'?($product -> ship == 1?"selected":""):"" }}>Có</option>
                    </select>
                    {!! $errors->first('ship', '<p class="help-block">:message</p>') !!}
                </div>
            </div>
        </div>
    </div>
