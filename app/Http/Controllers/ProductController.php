...
class ProductController extends Controller
{
	...
	public function detail($slug, $id)
	{
		...
		//get setting
		$setting = $this -> setting -> where('id',5) -> first();

		return view('frontend.product.detail',compact('product','products_like','products_related','categoryFeatures', 'reviews','setting','check_color','check_size'));
	}
	//recent view product
	public function viewRecentProduct()
    {
        $products = [100000];
        //kiem tra xem ton tai san pham gan day khong
        if(session()->has('products.recently_viewed')) {

	        $products = session()->get('products.recently_viewed');
	    }    

	    $product_recent = $this -> product -> whereIn('id', $products)->latest()->take(12)->get();
    	
        

        return view('frontend.product.recent_product',compact('product_recent'));
    }
    //change price
	...
	public function updateCartQuantity($id = null,$quantity = null){
		...
		DB::table('cart') -> where('id','['.$ids_string.']') -> increment('quantity',$quantity);
